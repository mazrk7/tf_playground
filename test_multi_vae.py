...
def plot_single_model(sess, model, dataset):
...
def test_multiple_models(sess, model, dataset, num_samples):    
    total_batch = int(num_samples / model.bs)
   
    # Loop over all batches
    for i in range(total_batch):
        batch_xs, batch_ys = dataset.next_batch(model.bs)
...
def main(seed): 
    ...
    with tf.Session() as sess:
        ...
        test_multiple_models(sess, vae, test, test.num_examples)
        ...
        #for index in range(NUM_CLASSES):
            ...
            #plot_single_model(sess, vae, test)
