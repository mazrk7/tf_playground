...
from fivo import base
from fivo import vrnn
...
def create_dataset_and_model(config, split, shuffle, repeat):
  ...
  if config.model == "vrnn":
    inputs, targets, lengths, mean = dataset.create_dataset(
        config.dataset_path, split, config.batch_size, shuffle=shuffle, repeat=repeat)

    # There is no bias for the generative distribution
...
def run_eval(config, create_dataset_and_model_fn=create_dataset_and_model):
  ...
  def create_graph():
    ...
    return ((elbo_ll, iwae_ll, fivo_ll), total_batch_length, batch_size, global_step)
...
def run_sample(config, create_dataset_and_model_fn=create_dataset_and_model):
  ...
  def sample_from_model(model, initial_state, initial_inputs, mean):
    ...
    Args:
      model: A model with sample_step implemented.
